...
public class revisedSimplex
{
  ...
  public void getRidOfArtificials()
	...
	if (varType[i] != Artificial) {
	  ...
	  case NonBasicType:
	    ...
	    break;
	  default:
	    System.out.println("Error: La variable debe ser de tipo basica o no basica");
	  }
	}
	...
	for (i = 0; i < numVariables; i++)  
	  ...
	  case BasicType:
	    if (varType[i] == Artificial) {
	      System.out.println("Error al deshacerse de las variables artificiales:");
	      System.out.println(" Toadavia hay variables artificiales");
	    }
	    BasicVariables[LastBasic] = i;
	    x[LastBasic] = TempX[i];
	    LastBasic++;
  ...
  public void showInfo()
	...
	System.out.println("costo["+j+"]:"+cost[j]);
      for (int i = 0; i < numConstraints; i++)
	for (int j = 0; j < numVariables; j++)
	  System.out.println("A["+i+"]["+j+"]:"+A[i][j]);
      
      System.out.println("Sacando Variable:"+LeavingVariable);
      
      System.out.println("Metiendo Variable:"+EnteringVariable);
      
      for (int i = 0; i < numConstraints; i++) 
	System.out.println("Costo reducido["+i+"]:"+littleCost[i]);
      
      
      System.out.println("Relacion minima:"+MinRatio);
	...
	System.out.println("yB["+i+"]:"+yB[i]);
      ...
      for(int i = 0; i < numConstraints; i++)
	System.out.println("Variables Basicas["+i+"]:"+BasicVariables[i]);
      
      for(int i = 0; i < numNonbasic; i++)
	System.out.println("Variables NO Basicas["+i+"]:" + 
			   NonBasicVariables[i]);
	...
	System.out.println("x["+i+"]:"+x[i]);
      ...
      for (int i = 0; i < numNonbasic; i++)
	System.out.println("Costos reducidos["+i+"]:"+reducedCost[i]);
      
      System.out.println("Valor del objetivo:"+objectiveValue);

      for (int i = 0; i < numVariables; i++)
	System.out.println("Cosoto original["+i+"]:" + 
			   OriginalCost[i]);

      System.out.println("Tipo de minimizacion "+ TypeMinimize);
      System.out.println("Tipo "+ oldOptType);
  ...
  public int AugmentBasis(int BasisSize)
    {
      ...
      if (rows > cols) {
	System.out.println("Restricciones irregulares en la matriz:");
	System.out.println("filas= "+rows+" columnas = "+cols);
	return 1;
      }
      
      if (BasisSize > rows) {
	System.out.println("Demasiados elementos en la base");
	System.out.println("Tamaño de la base = "+BasisSize+" rows = "+rows);
	return 1;
      }
	...
	if (BasicVariables[i] < 0 || BasicVariables[i] >= cols) {
	  System.out.println("Elementos bases["+i+"]= "+BasicVariables[i]+" fuera de rango");
	  return 1;
	}
      }
      
      if (BasisSize == rows) {
	System.out.println("La base esta llena");
	return 0;
      }
      perm = new int[cols]; 
